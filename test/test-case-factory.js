...
const copyFixture = function(name, dest) {
...
class TestCaseFactory {
    ...
    and(name, stepDefinition) {
        if (!this.context) {
            throw new Error("And used without context");
        }
    ...
    _build() {
        this.options.pageObjects = Boolean(this.pageObjects.length);
        this.options.customCommands = Boolean(this.customCommands.length);
        ...
        this.groups.forEach((group) => {
            if (group.name) {
                groupPath = path.join(this.testCasePath, "features", group.name);
            }
            else {
                groupPath = path.join(this.testCasePath, "features");
            }
