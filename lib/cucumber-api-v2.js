...
module.exports = class CucumberAPI {
  ...
  * _attach (stepResult, data, mimeType) {
    ...
    console.log('attaching screenshot', attachment)
  }
        ...
        if (
          ...
          code = function * () {
            try {
              ...
              yield * self.stepFailureHook(self._attach.bind(self, stepResult), feature, scenario)
            } catch (err) {
              console.error(err)
            }
  ...
  * createCucumberHtmlReport (options) {
    ...
    if (options.htmlReport) {
      console.log('Creating cucumber html report based on', report)
