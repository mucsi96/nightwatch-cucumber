  ...
  * takeScreenshot (moduleName, testName) {
  ...
  start () {
    return new Promise((resolve, reject) => {
      ...
      //   console.log('errorHandler', err, this.client.get().results, this.client.get().errors)
      //   reject(err)
      // }
      const completeHandler = () => {
        console.log('completeHandler', this.client.get().results, this.client.get().errors)
        restore()
        resolve()
      }
      const setup = () => {
        ...
        this.client.get().terminate = () => {
          restore()
          console.log('terminate', this.client.get().results, this.client.get().errors)
          const lastError = this.client.get().results.lastError
          const clientError = this.client.get().errors[0]

          this.clearResult()

          Nightwatch.queue.reset()
          Nightwatch.queue.empty()
          if (lastError) {
            reject(lastError)
            return
          }
        ...
        this.client.once('complete', completeHandler)
      }
