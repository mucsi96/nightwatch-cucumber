...
module.exports = class NightwatchApi {
  ...
  * takeScreenshot (moduleName, testName) {
  ...
  preventSessionClose () {
    if (typeof this.originalStartSession === 'undefined') {
      this.originalStartSession = this.client.get().options.start_session
      this.client.get().options.start_session = false
    }
  }

  restoreSessionClose () {
    this.client.get().options.start_session = this.originalStartSession
  }

  start () {
    return new Promise((resolve, reject) => {
      this.client.get().once('nightwatch:finished', (results) => {
        const resultErrors = results.tests.filter(test => test.failure).map(test => {
        ...
        if (resultErrors.length) {
          reject(combineErrors(resultErrors))
          return
        }
        resolve()
      })
      this.client.start()
      this.preventSessionClose()
    })
  }
