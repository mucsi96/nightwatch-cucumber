/* eslint-disable camelcase, no-use-before-define, new-cap */
"use strict";
...
module.exports = class NightwatchApi {
    ...
    * takeScreenshot(moduleName, testName) {
        ...
        const filePath = Nightwatch.Utils.getScreenshotFileName({
            module: moduleName,
            name: testName.replace(forbiddenCharacters, "") },
            true,
    ...
    * executeInNightwatchContext(fn, args) {
    ...
    enableShutdownForcing() {
        const orig = Nightwatch.ErrorHandler.handle;

        Nightwatch.ErrorHandler.handle = function(err, results, finished) {
            orig.call(this, err, results, function() {
                process.exit(!err ? 0 : 1);
            });
        };
    }

    addTestRunner(run, _undefined) {
        const self = this;
        ...
        Nightwatch.Runner.prototype.run = co.wrap(function * () {
            ...
            this.options.tag_filter = _undefined;
            ...
            try {
                const modules = yield new Promise((resolve, reject) => {
                    Nightwatch.Runner
                        .readPaths(self.testSource, innerSelf.options)
