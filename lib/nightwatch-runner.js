    ...
    return co(function * () {
      ...
      try {
        ...
        if (options.jsonReport) {
          mkdirp.sync(path.dirname(options.jsonReport))
          fs.writeFileSync(options.jsonReport, json)
        }
...
function getStepExecutor (options, browser, step) {
  return function * () {
    ...
    const result = yield new Promise((resolve) => {
      step.stepDefinitions[0].invoke(step, browser, {getAttachments: function () {}}, options.timeout, resolve)
    })

    if (result && result.getStatus() === Cucumber.Status.FAILED) {
      const err = result.getFailureException()
      yield * cucumber.logFailedStep(step, err.stack, true)
      client.handleException(err)
    }
  ...
  feature.discovered[scenarioName] = function (browser) {
    steps.forEach(function (step) {
      ...
      browser.perform((done) => {
        return co(getStepExecutor(options, browser, step)).then(done).catch((err) => done(err))
      })
    })
