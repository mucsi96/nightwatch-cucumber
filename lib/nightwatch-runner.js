...
function bootstrap (options) {
  ...
  Cucumber.Util.asyncForEach(runtime.getFeatures(), function (feature, nextFeature) {
    var backgroundSteps = []

    createTestFile(feature, options)

    if (feature.hasBackground && feature.hasBackground()) {
...
function discoverScenario (feature, scenario, steps, options) {
  ...
  if (scenario.getScenarioOutlineLine && scenario.getScenarioOutlineLine() || scenario.getLines && scenario.getLines().length > 1) {
