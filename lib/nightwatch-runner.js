  ...
  AsyncTree.runCommand = function(node, done) {
    return origRunCommand.call(this, node, function() {
      if (node.step && node.step.lastNode === node) {
        var lastResult = client.results.tests[client.results.tests.length - 1];
        logStepResult(node.step, lastResult);
      }
      done();
    });
  }
...
function logStepResult(step, result) {
  var stepResult = Cucumber.Runtime.StepResult({
    ...
    duration: 0,
    status: (result.failure ? Cucumber.Status.FAILED : Cucumber.Status.PASSED),
    failureException: ''
  });
