...
function patchNighwatchReporter () {
  ...
  Reporter.prototype.save = function () {
    ...
    return co(function * () {
      ...
      try {
        self.globalResults.cucumber = JSON.parse(fs.readFileSync(tempLogFile.name, 'utf8'))
      } catch (err) {
        console.error('Cannot reald cucumber json formatter output form ' + tempLogFile.name)
        console.error(err)
      }
  ...
  if (!feature.discovered) {
    feature.discovered = {
      ...
      beforeEach: function (browser, done) {
        return co(function * () {
          yield * logBeforeScenario(scenario)
          if (feature.hasBackground()) yield * logBackground(feature.getBackground())
        }).then(function () {
