...
module.exports = class Runner {
  ...
  * executeTestModules (dummyTestModules) {
    this.featureFiles = dummyTestModules.map((dummyTestModule) => this.dummyPathToFeaturePath(dummyTestModule))
    ...
    return yield * this.cucumberApi.run(this.featureFiles)
  }
  ...
  run (options) {
    ...
    this.jsonReport = this.cucumberApi.getJSONReportName()
    this.featureFiles = this.cucumberApi.getFeatureFiles()
    this.featureFiles.forEach((featureSource) => {
      try {
