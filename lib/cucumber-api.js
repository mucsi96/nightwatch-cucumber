...
const debug = require('debug')('nightwatch-cucumber')
...
module.exports = class CucumberAPI {
  ...
  log (event) {
    const eventLog = [event.getName()]
    const payload = event.getPayload()
    if (payload.getStatus) eventLog.push(payload.getStatus())
    if (payload.isSuccessful) eventLog.push(payload.isSuccessful() ? 'passed' : 'failed')
    debug(eventLog.join(' '))
  }
  ...
  * logPendingStep (step) {
    const pendingStepResult = Cucumber.Runtime.StepResult({ step: step, status: Cucumber.Status.PENDING })
    yield * this.logStepResult(pendingStepResult)
  ...
  * addAttachment (data, mimeType) {
    this.attachments.push(Cucumber.Runtime.Attachment({ mimeType, data }))
  }
